Shader "Unlit/Test5"
{
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            float heart(float2 st,float n )
            {
                // 位置とか形の調整
                st = (st - float2(0.5, 0.38)) * (1 * float2(2.1, 2.8)) / n;
            ...
            float wave(float2 st, float n)
            {
                ...
                return (1 + sin(d * 3 - (_Time.y * 3) + offs)) * 0.5;
            }
            ...
            float4 frag(VertexOutput output) : SV_Target 
            {
                ...
                float size = wave(output.uv, n);
                return (step(heart(st,size), abs(sin( 8 - _Time.w / 2))) ? _HeartColor : 1 );
            }
            ENDCG
        }
    }
